...
const ReadComments = async(count:number=0,postId:string) => {
      let response = await Axios.post<Iapi<IcommentState> >(`/comments/${postId}`,{},{headers:{"count":count}});
...
export const ReadCommentsFn = async<T>(data:IcommentState,dispatch:T,postId:string) => {
    let response = await Axios.post<Iapi<IcommentState>>(`/comments/${postId}`,{},{headers:{"count":data.count}})
